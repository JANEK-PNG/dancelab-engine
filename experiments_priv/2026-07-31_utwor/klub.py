"""Klub — utwór celowany w ZMIERZONY profil dwunastu referencji.
...
STEMY = ("KICK", "SUB", "DRUMS", "HATS", "CHORDS", "LEAD", "TEXTURES")
...
def hat(otwarty=False, seed=0):
    ...
    for f in (3180, 4210, 5310, 6790, 8330, 10250, 12400):
        ...
        if fr < SR / 2 * 0.95:
            ...
            y += sosfilt(tf2sos(b, a), z)     # NIGDY (b,a): bieguny > 1 przy 96k
...
AKORDY = [("D", 1), ("F", 1), ("C", 1), ("G", 1)]   # REALNY sub: 36–49 Hz
...
def buduj(rng):
    ...
    tex[:, 0] += (pow_ * lfo * 0.05).astype(np.float32)
    tex[:, 1] += (np.roll(pow_, 1301) * lfo * 0.05).astype(np.float32)
    for _ in range(int(TOTAL * 1.5)):                  # trzaski (było 9/s)
...
MIX = {
    ...
    "HATS":     (3000, None, 0.00, 0.08, -31.0, 14.0),
    "CHORDS":   (140, 6500, 0.20, 0.26, -21.5, 14.0),
    "LEAD":     (260, 12000, 0.15, 0.30, -29.0, 14.0),
    "TEXTURES": (200, None, 0.05, 0.22, -33.0, 16.0),
}
...
def main():
    ...
    p.add_argument("-o", "--output", default="klub")
