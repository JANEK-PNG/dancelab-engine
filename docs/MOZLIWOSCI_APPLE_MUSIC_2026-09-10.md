...
3. **Odsłuch strumieni w oknie.** Najpierw 30-minutowy test: MusicKit JS w WKWebView (pywebview) — gra czy nie. Jeśli gra: odsłuch pełnych utworów dla 82 % kolekcji. Jeśli nie: zostaje 30 s i trzeba to powiedzieć. **Zmierzone 2026-09-10 (`experiments_priv/2026-09-10_musickit_okno/`): gra.** Pełny utwór w oknie pywebview na `file://`, zalogowanym tokenem Janka przekazanym przez adres strony. Otwarte: kraj sklepu (MusicKit bez podanego kraju bierze `us`, Janek ma `pl`) i wpięcie do prawdziwego okna. **Wpięte tego samego dnia (`gui/statyczne/apple_odtwarzacz.js`):** strumienie grają w oknie, kraj sklepu z konta DJ-a, tokeny przez `js_api`; skoki o uderzenia i szew na strumieniu jeszcze nie.
