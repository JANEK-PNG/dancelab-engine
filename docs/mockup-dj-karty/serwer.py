"""Serwer systemu VJ, który NIE POZWALA się buforować.
...
def main() -> None:
    port = int(sys.argv[1]) if len(sys.argv) > 1 else 8649
    with http.server.ThreadingHTTPServer(("127.0.0.1", port), BezCache) as srv:
        print(f"karty DJ bez cache na http://localhost:{port}", flush=True)
        srv.serve_forever()
