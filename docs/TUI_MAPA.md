...
| Odsłuch wg STANDARDU graczy (weto Janka 06.08 na wynajdywanie koła): SPACJA=graj/pauza (P alias; priority+check_action puszcza spację do pól i przycisków), GDY GRA ↓/↑=next/previous (wzorzec Quick Look; debounce 0,25 s, zero nakładki; przy pauzie strzałki tylko zaznaczają — tryb Shift+P SKASOWANY, stan odtwarzacza JEST trybem), →/← skok ±8 uderzeń z tempa siatki tylko gdy gra; `tui/odtwarzacz.py` na ffplay (pauza z pozycją, wznowienie); pasek szwu (C) → spacja gra przejście; degradacja bez ffplay jawna — `tui/seam_preview.zbuduj_szew` na maszynerii `preview/transition_simulation` (fraz-lock, cache); dźwięk TYLKO z jawnego klawisza, drugie P/Esc stop, koniec z aplikacją | P w zakładce Set · ▶ w #compare |
