...
| P KONTEKSTOWE (Janek 06.08): pasek szwu zamknięty → sam zaznaczony utwór (afplay); pasek otwarty (C) → przejście pary; drugie P stop, C zamyka; przycisk ▶ = to samo — `tui/seam_preview.zbuduj_szew` na maszynerii `preview/transition_simulation` (fraz-lock, cache); dźwięk TYLKO z jawnego klawisza, drugie P/Esc stop, koniec z aplikacją | P w zakładce Set · ▶ w #compare |
