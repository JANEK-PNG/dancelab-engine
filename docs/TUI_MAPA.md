...
| G / przycisk „→ Zbuduj z filarów”: SZKIC w Set (złote ⚑, BEZ budowy — brief zostaje w grze; decyzja Janka 05.08), B buduje wokół: `_rozstaw_filary` → `locked_positions` silnika (równomierne pozycje, rosnąco po BPM — metafora inżynierska Janka: filar PODPIERA konstrukcję, nie leży na ogonie; v1 nie patrzy w krzywą łuku peak); flagi ⚑ z ctx.filary (id po mapowaniu duplikatów) także w gotowym secie; odmowa z liczbą przy <3; filary → `build_set(pinned_track_ids=…)` (istniejący mechanizm silnika); `_filary_for_build` pomija imiennie filar spoza puli/okna tempa, odmawia gdy filarów > miejsc; `dedup.canonical_ids` mapuje filar-duplikat na egzemplarz kanoniczny | budowa w zakładce Set „wokół N filarów" |
