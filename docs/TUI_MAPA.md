...
| ARTWORK sync `ingestion/artwork_sync` (przycisk w Bibliotece, worker w tle): iTunes Search API → PEWNE dopasowanie artysta+tytuł (niejednoznaczne=pominięte imiennie — zła okładka na CDJ to obciach) → 600×600 do tagów (APIC/covr/FLAC picture) z weryfikacją odczytem i ZACHOWANIEM mtime (cache nie padają); raport JSON; potem Reload Tags w RB → CDJ; klik AUTO-włącza galerię K (użytkownik ma widzieć efekt); guzik „Artwork sync" mały POD SZUKAJKĄ, nie przy Analizuj (dodatek, nie killer feature — Janek); DanceLab.command sam przenosi się do Ghostty z Terminala (ostre okładki) | przycisk Artwork sync |
