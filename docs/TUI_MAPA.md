...
| C = porównanie pary od dołu (krok d wizji ✅): `zaplanuj_szew` (wspólny plan z P — okna/cue/długość silnika, zero audio) + `pasek_energii` (RMS z ramek analizy, złote okno szwu, braki jawne) — wzorzec Export RB; „graj oba" = P (sync/kwantyzacja w renderze z natury) | klawisz C → panel #compare, drugie C/Esc chowa |
